...
function MyFavorites({ showAlert, alert }) {
  ...
  const deleteCocktail = async (deleteId = false) => {
    if (deleteId) {
      await axios.delete(`http://localhost:3000/myfavorites/${deleteId}`);
      console.log("deleted one");
    } else {
      try {
        await allMyFavoritesData.map(async (cocktail) => {
          await axios.delete(
            `http://localhost:3000/myfavorites/${cocktail.id}`
          );
        ...
        {allMyFavoritesData ? (
          allMyFavoritesData.length > 0 ? (
            allMyFavoritesData.map((drink, i) => (
              <div
                key={i}
                className="my-coctail-box"
                title={drink.name}
                onClick={() => console.log(drink.id)}
              >
                <div
                  className="delete-cocktail"
                  onClick={() => {
                    deleteCocktail(drink.id);
                    showAlert("danger show", `${drink.name} </bold>deleted.`);
                  }}
                >
